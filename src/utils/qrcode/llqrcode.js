...
function _db (_fa) {
  ...
  this.decode = function (received, _fv) {
    ...
    var noError = true
    for (var i = 0; i < _fv; i++) {
      var eval2 = poly.evaluateAt(this._fa.exp(_fq ? i + 1 : i))
      _dh[_dh.length - 1 - i] = eval2
      if (eval2 != 0) {
        noError = false
      }
    }
...
qrcode.grayScaleToBitmap = function (f) {
  ...
  for (var i = 0; i < b; i++) {
    for (var a = 0; a < b; a++) {
      for (var g = 0; g < d; g++) {
        for (var h = 0; h < e; h++) {
          c[e * a + h + (d * i + g) * qrcode.width] = (f[e * a + h + (d * i + g) * qrcode.width] < j[a][i])
        }
      }
    }
  }

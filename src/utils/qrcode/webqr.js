...
function read (a) {
  var html = '<br>'
  if (a.indexOf('http://') === 0 || a.indexOf('https://') === 0) { html += "<a target='_blank' href='" + a + "'>" + a + '</a><br>' }
...
function success (stream) {
  if (webkit) { v.src = window.webkitURL.createObjectURL(stream) } else
  if (moz) {
...
function setimg () {
  setTimeout(() => {
    document.getElementById('codeInput').onchange = function (e) {
